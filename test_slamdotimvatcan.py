...
def estimate_near(box, contour_area, frame_area, H_):
    x, y, w, h = box
    bottom = (y + h) / float(H_)
    h_ratio = h / float(H_)
...
# =========================
def compute_sector_states(W_, H_, tracks_, camera_blocked, ultrasonic_cm_):
    ...
    for t in tracks_:
        ...
        x, y, w_, h_ = t.box
        cx = x + w_ / 2.0
        idx = int(cx / float(W_) * SECTOR_N)
        idx = max(0, min(SECTOR_N - 1, idx))
        sector_states[idx] = "blocked"

    mid = SECTOR_N // 2
    if ultrasonic_cm_ is not None:
        if ultrasonic_cm_ < SAFE_DIST_CM:
...
def draw_mini_map(frame, sector_states, camera_blocked, ultrasonic_cm_):
    """
    Mini-map dạng radar:
      - Robot là tam giác trắng ở giữa.
      - Sector 'free' = hình quạt màu xanh.
      - Sector 'blocked' = chấm đỏ ở vòng ngoài.
      - Camera bị che = vòng tròn đỏ + dấu X.
    """
    h, w, _ = frame.shape

    # Kích thước mini-map hình vuông
    MAP_SIZE = 160
    cx = cy = MAP_SIZE // 2
    radius = MAP_SIZE // 2 - 6
    inner_r = 20  # bán kính trong (không tô để nhìn rõ robot)
    ...
    # Vẽ vòng tròn biên
    cv2.circle(map_img, (cx, cy), radius, (180, 180, 180), 2)

    # Lưới nhẹ (4 tia chia góc)
    for ang_deg in [-45, -15, 15, 45]:
        ang = math.radians(ang_deg)
        x2 = int(cx + math.sin(ang) * radius)
        y2 = int(cy - math.cos(ang) * radius)
        cv2.line(map_img, (cx, cy), (x2, y2), (70, 70, 70), 1)

    # Nếu camera bị che -> vẽ dấu X đỏ, không vẽ gì thêm
    if camera_blocked:
        cv2.circle(map_img, (cx, cy), radius - 4, (0, 0, 200), 2)
        cv2.line(map_img, (cx - radius + 10, cy - radius + 10),
                 (cx + radius - 10, cy + radius - 10), (0, 0, 255), 3)
        cv2.line(map_img, (cx + radius - 10, cy - radius + 10),
                 (cx - radius + 10, cy + radius - 10), (0, 0, 255), 3)
    else:
        # FOV phía trước (ví dụ ~100 độ, giống camera trước mặt)
        FOV_DEG = 100.0
        n = max(1, len(sector_states))

        for i, state in enumerate(sector_states):
            # Góc bắt đầu/kết thúc của sector
            start_deg = -FOV_DEG / 2.0 + (i * FOV_DEG / n)
            end_deg = -FOV_DEG / 2.0 + ((i + 1) * FOV_DEG / n)
            ...
            # Tô vùng đi được = sector 'free' -> hình quạt màu xanh
            if state == "free":
                pts = []
                # vòng trong
                pts.append((
                    int(cx + math.sin(start_rad) * inner_r),
                    int(cy - math.cos(start_rad) * inner_r)
                ))
                pts.append((
                    int(cx + math.sin(end_rad) * inner_r),
                    int(cy - math.cos(end_rad) * inner_r)
                ))
                # vòng ngoài
                pts.append((
                    int(cx + math.sin(end_rad) * radius),
                    int(cy - math.cos(end_rad) * radius)
                ))
                pts.append((
                    int(cx + math.sin(start_rad) * radius),
                    int(cy - math.cos(start_rad) * radius)
                ))
                cv2.fillPoly(map_img, [np.array(pts, dtype=np.int32)], (0, 170, 0))

            # Vật cản = chấm đỏ ở vòng ngoài
            if state == "blocked":
                px = int(cx + math.sin(mid_rad) * (radius - 10))
                py = int(cy - math.cos(mid_rad) * (radius - 10))
                cv2.circle(map_img, (px, py), 6, (0, 0, 255), -1)

        # Vòng hiển thị khoảng cách sonar (nếu có)
        if ultrasonic_cm_ is not None:
            # map khoảng cách về [inner_r+5, radius-8]
            r_min = inner_r + 5
            r_max = radius - 8
            ratio = ultrasonic_cm_ / float(SAFE_DIST_CM)
            ratio = max(0.15, min(1.2, ratio))
            r_sonar = int(r_min + (r_max - r_min) * min(1.0, ratio))
            cv2.circle(map_img, (cx, cy), r_sonar, (0, 120, 120), 1)

    # Robot ở giữa: tam giác trắng hướng lên
    robot_pts = np.array([
        (cx, cy - inner_r + 2),
        (cx - 8, cy + 8),
        (cx + 8, cy + 8),
    ], dtype=np.int32)
    cv2.fillPoly(map_img, [robot_pts], (255, 255, 255))
    cv2.polylines(map_img, [robot_pts], True, (0, 0, 0), 1)

    # Overlay mini-map lên góc phải trên màn hình
    ...
    # nền đen + viền
    cv2.rectangle(frame, (x1 - 4, y1 - 4), (x2 + 4, y2 + 24), (0, 0, 0), -1)

    frame[y1:y2, x1:x2] = map_img

    cv2.putText(frame, "Radar mini-map", (x1, y2 + 16),
...
def camera_loop():
    ...
    mask_floor, poly = build_trapezoid_mask(H, W, y_top=TRAP_Y_TOP,
                                            top_ratio=TRAP_TOP_RATIO,
                                            bottom_ratio=TRAP_BOTTOM_RATIO)
    ...
    while True:
        ...
        cnts_full, _ = cv2.findContours(full_edges, cv2.RETR_EXTERNAL,
        ...
        camera_blocked = (max_bbox_cover_full >= BLOCK_BOX_TH) or \
                         (lap_var < BLUR_TH and edge_ratio_full < EDGE_LOW_TH)
        ...
        # Thanh sector phía dưới (debug)
        draw_sector_bar_bottom(frame, sector_states)

        # Mini-map dạng radar: robot ở giữa, xanh = đi được, đỏ = vật cản
        draw_mini_map(frame, sector_states, camera_blocked, ultrasonic_cm)
